      ...
      <html lang="en" data-theme="light">
        ...
        <body class="bg-base-100 text-base-content">
          <div class="flex flex-col min-h-screen">
            <header class="shadow-lg navbar bg-base-200">
              <div class="navbar-start">
                <a class="text-xl btn btn-ghost" href="/">
                  <img src=config.logo alt="Logo" class="mr-2 w-8 h-8" />
                  {config.website_name}
                </a>
              </div>
            ...
            <main class="container flex-1 py-8 px-4 mx-auto">{children()}</main>
            ...
            <footer class="p-10 footer footer-center bg-base-200 text-base-content">
              <aside>
                ...
                {config.website_tagline.map(|tagline| view! { <p>{tagline}</p> })}
...
pub fn index(
    ...
    view! {
      <Layout config=config additional_js=additional_js>
        <div class="mb-16 bg-gradient-to-r hero from-primary to-secondary text-primary-content rounded-box">
          <div class="py-20 text-center hero-content">
            <div class="max-w-md">
              <h1 class="mb-5 text-5xl font-bold">{config.website_name}</h1>
              {if !config.website_tagline.is_empty() {
                view! { <p class="mb-5 text-xl">{config.website_tagline}</p> }.into_any()
              } else {
                view! {}.into_any()
              }}
        ...
        <section id="content">
          <h2 class="mb-12 text-3xl font-bold text-center">"Latest Content"</h2>
                  ...
                  <div class="shadow-xl card bg-base-200">
                    <div class="card-body">
                      <h3 class="card-title">{content_item.meta().get_title()}</h3>
                      {
                        ...
                        if !desc.is_empty() {
                          view! { <p class="text-base-content/70">{desc}</p> }.into_any()
                        } else {
                          view! {}.into_any()
                        }
                      }
...
pub fn content(
    ...
    Ok(view! {
      <Layout config=config additional_js=additional_js>
        <article class="mx-auto max-w-4xl prose prose-lg">
          <header class="mb-8 text-center">
            <h1 class="mb-4 text-4xl font-bold text-primary">{content_item.meta().get_title()}</h1>

            <div class="flex flex-wrap gap-4 justify-center text-sm text-base-content/70">
              {if let Some(author) = content_item.meta().get_author() {
                view! { <span>"By " {author}</span> }.into_any()
              } else {
                view! {}.into_any()
              }}
              {if let Some(date) = content_item.meta().get_date() {
                view! { <span>{date}</span> }.into_any()
              } else {
                view! {}.into_any()
              }}
                ...
                view! {
                  <div class="flex gap-2">
                    {content_item
                      ...
                      .tags
                      .iter()
                      .map(|tag| view! { <span class="badge badge-primary">{tag}</span> })
                      .collect::<Vec<_>>()}
                  </div>
                }
                  .into_any()
          ...
          <footer class="pt-8 mt-12 border-t border-base-300">
